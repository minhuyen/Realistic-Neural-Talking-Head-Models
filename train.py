    ...
    for i_batch, (f_lm, x, g_y, i, W_i) in enumerate(pbar, start=0):
        ...
        if i_batch % 1 == 0:
            with torch.autograd.enable_grad():
                ...
                # BxK,2,3,224,224
                f_lm_compact = f_lm.view(-1, f_lm.shape[-4],
                                         f_lm.shape[-3], f_lm.shape[-2], f_lm.shape[-1])

                e_vectors = E(f_lm_compact[:, 0, :, :, :],
                              f_lm_compact[:, 1, :, :, :])  # BxK,512,1
                # B,K,512,1
                ...
                lossG = criterionG(x, x_hat, r_hat, D_res_list,
                                   D_hat_res_list, e_vectors, D.module.W_i, i)
        ...
        if i_batch % 1 == 0 and i_batch > 0:
            ...
            pbar.set_postfix(epoch=epoch, r=r.mean().item(),
                             rhat=r_hat.mean().item(), lossG=lossG.item())
            ...
            if display_training:
                ...
                for img_no in range(1, x_hat.shape[0]//16):
                    out = torch.cat(
                        (out, (x_hat[img_no]*255).transpose(0, 2)), dim=1)
                ...
                for img_no in range(1, x.shape[0]//16):
                    out = torch.cat(
                        (out, (x[img_no]*255).transpose(0, 2)), dim=1)
                out = out.type(torch.int32).to(cpu).numpy()
                ...
                for img_no in range(1, g_y.shape[0]//16):
                    out = torch.cat(
                        (out, (g_y[img_no]*255).transpose(0, 2)), dim=1)
                out = out.type(torch.int32).to(cpu).numpy()
